...
class_name VisionImageInfo extends RefCounted

const DATA_BUFFER_PROPERTY := &"buffer"
const DATA_WIDTH_PROPERTY := &"width"
const DATA_HEIGHT_PROPERTY := &"height"
const DATA_FORMAT_PROPERTY := &"format"
const DATA_HAS_MIPMAPS_PROPERTY := &"has_mipmaps"
...
static func create_from_image(a_image: Image) -> VisionImageInfo:
	return VisionImageInfo.new(
		{
