...
func _on_vision_face_mesh_failed(error: VisionScanError) -> void:
	_print_to_screen("Face mesh failed")
...
static func _draw_mustache(a_image: Image, a_face: FaceMeshInfo, a_w: int, a_h: int) -> void:
	...
	for i in range(last + 1):
		...
		_mustache_stroke(a_image,
				Vector2i(int(spine[i].x),     int(spine[i].y)),
				Vector2i(int(spine[i + 1].x), int(spine[i + 1].y)),
				r, Color.BLACK, a_w, a_h)


# Stamps filled discs along the segment a_from→a_to to produce a thick stroke.
static func _mustache_stroke(a_image: Image, a_from: Vector2i, a_to: Vector2i,
		a_radius: int, a_color: Color, a_w: int, a_h: int) -> void:
	...
	for i in range(steps + 1):
		...
		_mustache_disc(a_image,
				Vector2i(int(lerpf(float(a_from.x), float(a_to.x), t)),
						 int(lerpf(float(a_from.y), float(a_to.y), t))),
				a_radius, a_color, a_w, a_h)


# Fills every pixel within a_radius of a_centre with a_color.
static func _mustache_disc(a_image: Image, a_centre: Vector2i,
		a_radius: int, a_color: Color, a_w: int, a_h: int) -> void:
...
static func _draw_glasses(a_image: Image, a_face: FaceMeshInfo, a_w: int, a_h: int) -> void:
	...
	var l_min := Vector2(INF, INF);  var l_max := Vector2(-INF, -INF)
	var r_min := Vector2(INF, INF);  var r_max := Vector2(-INF, -INF)
	...
	if l_min.x > r_min.x:
		var tmp_min := l_min; var tmp_max := l_max
		l_min = r_min;        l_max = r_max
		r_min = tmp_min;      r_max = tmp_max
...
static func _draw_cowboy_hat(a_image: Image, a_face: FaceMeshInfo, a_w: int, a_h: int) -> void:
	...
	var o_min_x: float = INF;  var o_max_x: float = -INF;  var o_min_y: float = INF
	for pt: Vector3 in oval_pts:
		var px: float = pt.x * a_w;  var py: float = pt.y * a_h
		if px < o_min_x: o_min_x = px
		if px > o_max_x: o_max_x = px
		if py < o_min_y: o_min_y = py
...
static func _fill_ellipse(a_image: Image, cx: float, cy: float,
		rx: float, ry: float, a_color: Color, a_w: int, a_h: int) -> void:
	if rx <= 0.0 or ry <= 0.0:
		return
...
static func _blend_ellipse(a_image: Image, cx: float, cy: float,
		rx: float, ry: float, a_color: Color, a_w: int, a_h: int) -> void:
	if rx <= 0.0 or ry <= 0.0:
		return
	for y in range(int(cy - ry), int(cy + ry) + 1):
		for x in range(int(cx - rx), int(cx + rx) + 1):
			...
			if (dx * dx) / (rx * rx) + (dy * dy) / (ry * ry) <= 1.0:
				if x >= 0 and x < a_w and y >= 0 and y < a_h:
					...
					a_image.set_pixel(x, y, existing.lerp(
							Color(a_color.r, a_color.g, a_color.b, 1.0), a_color.a))
